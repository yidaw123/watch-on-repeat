...
<script>
  ...
  function openShareModal(url, title) {
    ...
    document.getElementById('blog-share-modal').style.display = 'flex';
  }
  ...
  function shareSocial(network) {
    ...
    const title = encodeURIComponent(activeShareTitle);
    let shareUrl = '';
...
// Insert the modal HTML right before </body>
content = content.replace(/<\/body>\s*<\/html>/, modalHtml + '</body>\n</html>');

// Replace the navigator.share block with openShareModal
const badBlock = `if (navigator.share) {
                navigator.share({ title: title, url: url }).catch(console.error);
              } else {
                navigator.clipboard.writeText(url).then(() => alert("Link copied to clipboard!"));
              }`;
content = content.replace(badBlock, `openShareModal(url, title);`);

fs.writeFileSync(filePath, content, 'utf8');
console.log("Updated index.html with modal!");
