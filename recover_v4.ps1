...
public class Replayer {
    public static void ApplyChunks(string filePath, string[] targets, string[] replacements, int[] starts, int[] ends, bool[] allows) {
        ...
        foreach (var chunk in chunks) {
            ...
            if (startIdx > endIdx) {
                Console.WriteLine($"[!] Invalid range {startLine}-{endLine} in {filePath}");
                continue;
            }
            ...
            if (regionText.Contains(target)) {
                ...
                if (globalIdx >= 0) {
                    Console.WriteLine($"[*] Target globally found for {startLine}-{endLine}");
            ...
            if (!matched) {
                Console.WriteLine($"[!] Target content not found in {filePath} at {startLine}-{endLine}");
                continue;
            }
                    ...
                    if ($target -match '(app\.js|index\.html|style\.css)$') {
                        if ($name -eq 'write_to_file') {
                            ...
                            if ($name -eq 'replace_file_content') {
                                ...
                                $allows += [bool](if ($null -ne $args.AllowMultiple) { $args.AllowMultiple } else { $false })
                            }
                            elseif ($name -eq 'multi_replace_file_content') {
                                foreach ($rc in $args.ReplacementChunks) {
                                    ...
                                    $ends += [int]$rc.EndLine
                                    $allows += [bool](if ($null -ne $rc.AllowMultiple) { $rc.AllowMultiple } else { $false })
                                }
                            }
